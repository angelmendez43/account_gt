...
class LibroCompras(models.AbstractModel):
    ...
    def _get_compras(self,datos):
        ...
        compra_ids = self.env['account.invoice'].search([('company_id','=',self.env.user.company_id.id),('date_invoice','<=',datos['fecha_fin']),('date_invoice','>=',datos['fecha_inicio']),('state','in',['open','paid']),('type','in',['in_invoice','in_refund'])])
