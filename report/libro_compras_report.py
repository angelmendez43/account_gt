...
class LibroCompras(models.AbstractModel):
    ...
    def _get_compras(self,datos):
        compras_lista = []
        gastos_no_lista = []
        compra_ids = self.env['account.move'].search([('company_id','=',self.env.user.company_id.id),('date','<=',datos['fecha_fin']),('date','>=',datos['fecha_inicio']),('state','=','posted'),('type','in',['in_invoice','in_refund'])])
        ...
        if compra_ids:
                ...
                for compra in compra_ids:
                    if 'RECIBOS' not in compra.journal_id.name:
